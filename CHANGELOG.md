...
- Change verification now **fails closed**. When git cannot answer — no work tree, no commits, git unavailable, or more untracked files than can be hashed — the report says `UNVERIFIED` instead of skipping the check. The previous behaviour skipped, on the reasoning that a false refusal is worse than no check; that was backwards, because it made `ready` obtainable in a tree where nothing had been verified. `verifyChanges: false` is the explicit waiver.
- The workspace fingerprint covers untracked file **contents**, not merely their names. A stash tree holds tracked files only and `status` carries untracked paths without their bytes, so two different untracked files fingerprinted identically — a stale pass indistinguishable from a fresh one for any untracked file. Contents are hashed through one `git hash-object` call, and exceeding the argv-safe path cap reports unverified rather than silently degrading to names.
- A completeness check: the claim is now compared against every path the work tree reports as changed, and an omission is reported as `UNDECLARED_CHANGE`. A completion that names the files it wants judged and stays silent about the rest is true as far as it goes and misleading about the whole — precisely the shape a self-reported claim takes. Configurable with `verifyComplete`; applies only inside a git work tree.
- `workspaceDigest` on every report: a content-sensitive fingerprint of the work tree the verdict was taken against, so a reader can tell later whether the code moved. Without it a stale pass is indistinguishable from a fresh one. Computed with `git stash create` — which writes a commit object for the current tree without touching the stash list or the working tree — folded together with `HEAD` and `git status` to cover untracked names.
- The README now names the two gaps that cannot be closed from inside this plugin, and the Harness events that would close them: `agent/inbox/claimed` (or `session/event` filtered on `source.kind`) for capturing the user's actual requirement so the agent is handed only a task ID, and `agent/turn-stopping` — a serial, awaited turn-close event — for enforcement rather than instruction.
