...
|---|---|
| Run shell commands | Only the exact strings in `verificationCommands`, each executed as `/bin/sh -lc <command>` with the working directory set to `workspaceRoot` |
| Inspect claimed files | Existence only, and only inside `workspaceRoot` and the listed `allowedPathPrefixes` |
...
- **Read file contents.** The file policy calls `access()` only; it never opens a claimed file. Reading a file's bytes is not a capability this plugin has.
